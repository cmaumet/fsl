...
int fmrib_main(int argc, char *argv[])
{
  ...
  vector<cluster<T> > originalClusters;
  get_stats(labelim,zvol,originalClusters,minv.value());
  int nOriginalLabels(originalClusters.size()+1); //0 is also a label of sorts
  if (verbose.value()) cout<<"Number of labels = "<<originalClusters.size()<<endl;

  // process the cope image if entered
  vector<cluster<T> > originalCopeClusters;
  if (!copename.unset()) {
